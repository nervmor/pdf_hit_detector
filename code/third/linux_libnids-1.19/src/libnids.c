...
*/
	...
	// [custom_fix]
	// 当数据包为ip分片包(ip包最大长度)时，有些网卡会对ip包的total_len字段修改为0
	// 导致libnids判定为无效包而丢弃
	// 这里自动修复ip包的total_len字段
	iph = (struct ip *)data_aligned;
	{
		int actual_len = hdr->len - linkoffset;
		if (ntohs(iph->ip_len) == 0)
		{
			iph->ip_len = htons(actual_len);
		}
	}
	if (hdr->len != hdr->caplen)
	{
		printf("-----------[hdr->len(%d) != hdr->caplen(%d)]-----------\n\n", hdr->len, hdr->caplen);
	}
...
static int open_live()
{
	...
	// 设置捕获的最大包的长度为 10M, 一般情况下不会有超过10M的包，如果有就再加大，以免造成捕获的包的内容不全
    if ((desc = pcap_open_live(device, 10 * 1024 * 1024, promisc,
...
#ifdef _WIN32
	// 设置缓冲区大小为100M(如果缓冲区太小，当大量数据传输(大文件下载)时会造成丢包)
	pcap_setbuff(desc, 100*1024*1024);
#endif // _WIN32
    init_procs();
